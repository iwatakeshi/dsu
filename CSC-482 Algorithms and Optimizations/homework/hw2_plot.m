...
figure(1);
compare(1:5, @factorial, @exp, 'PlotMethod', 'loglog');

figure(2);
compare(1:10^3, @factorial, @exp, 'PlotMethod', 'loglog');
...
figure(11);
compare(0:0.5:2, f1, f2);
ylim([0 4]);

figure(12);
compare(0:0.5:2^18, f1, f2);

% vii. log2(n)  vs.  ln(n)  vs log10(n)

figure(13);
compare2(1:0.5:20, @(n) log2(n), @(n) log(n), @(n) log10(n));
ylim([1 5]);
...
figure(16);
compare(1:0.5:2^15, @(n) log2(n), @(n) log2(log2(n.^5)));
ylim([0 20]);

% ix. n^1.5 vs. 1000 *n * log2(n)

figure(17);
compare(0:0.5:2^3, @(n) n.^5, @(n) 1000 * n .* log2(n));

figure(18);
compare(0:0.5:2^4, @(n) n.^5, @(n) 1000 * n .* log2(n));

% x. n^.5 vs 99999 * log2(n)

figure(19);
compare(0:0.01:2, @(n) n.^(0.5), @(n) 99999 * log2(n));
ylim([0 10]);

figure(20);
compare(0:0.01:2^8, @(n) n.^(0.5), @(n) 99999 * log2(n));

% xi. n.^(0.2) vs 99 * log2(n)

figure(21);
compare(0:0.05:10, @(n) (power(n, 0.2)), @(n) (99 * log2(n)));
ylim([0 2])

figure(22);
compare(0:0.05:10^5, @(n) (power(n, 0.2)), @(n) (99 * log2(n)));
