...
void
displaySolution(int nNodes, int root, Path* solution, double elapsedTime);

// algorithm functions (not defined here)
...
int main() {
  ...
  stop = clock();
  displaySolution(n, root, bruteSolution, (stop - start) / (double)CLOCKS_PER_SEC);

  // start = clock();
  greedySolution = TSPGreedy(n, root, G);
  // stop = clock();
  displaySolution(n, root, greedySolution, (stop - start) / (double)CLOCKS_PER_SEC);
  freem(G, n);
  return 0;
}

void displaySolution(int n, int root, Path* solution, double elapsedTime) {
  cout << "\n\nRoot: " << root << "\n";
...
Path* TSPBruteWorker(int start, int end, int tour_length, int tours[], int** G) {
  ...
  if (tour_length == 1) { // base case: only 1 tour node between the start/end nodes
    ...
    // printf("tours: \n");
    // for (int k = 0; k < tour_length; k++) {
    //   printf("%lld ", G[start][tours[k]]);
    // }
    // printf("\n");
    ...
    // printf("remaining tours:\n");
    // for (int k = 0; k < tour_length - 1; k++) {
    //   printf ("%d ", remaining_tours[k]);
    // }
    // printf("\n");
    // printf("Cycle through tours:\n");
    ...
    for (int i = 0; i < tour_length; i++) {
      ...
      // printf("Sub tour: \n");
      sub_tour = TSPBruteWorker(current_node, end, tour_length - 1, remaining_tours, G);
...
Path* TSPBrute(int n, int root, int** G) {
  ...
  solution = TSPBruteWorker(root, root, n - 1, tours, G);
  return solution;
}
