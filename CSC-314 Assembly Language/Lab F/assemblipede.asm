...
section .data
    ...
    instructionsFmt db "%s%s%s", 0x00
    scoreFmt db "Score: %d", 0x0A, 0x00
    debugCharFmt db "%d", 0x0A, 0x00
section .bss
    lvlFilePtr resb 4
...
_GameTick:
    ...
    _pause:
        ; call clear
        push instructionsMsg3
        push instructionsMsg2
        push instructionsMsg1
        push instructionsFmt
        call printw
        add esp, 16
    _pause.loop:
        call getch
    ...
    _skipWrapTailIndex: 
        ...
        je _eatStar
        cmp byte[eax], 45      ; check '-'
        je _GameOver
        cmp byte[eax], 124     ; check '|'
        je _GameOver
